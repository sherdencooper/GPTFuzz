...
def main(args, args_target, MODEL, TOK, MODEL_TARGET, TOK_TARGET, questions, initial_seed):
    ...
    status = fuzzing_status(questions, initial_seed=initial_seed, max_query=args.max_query, energy=args.energy, seed_selection_strategy=args.seed_selection_strategy, mode='multi-single')
    counter = 0
    save_len = len(status.seed_queue)
    if args.initial_seed_filter:
        save_path = './datasets/prompts_generated/multi_single/multi_single_' + target_model_name + '_' + args.filter_method + '.csv'
    elif args.seed_selection_strategy != 'MCTS':
        save_path = './datasets/prompts_generated/multi_single/multi_single_' + target_model_name + '_' + args.seed_selection_strategy + '.csv'
    else:
        save_path = './datasets/prompts_generated/multi_single/multi_single_' + target_model_name + '.csv'
    # delete the file if it exists
    if os.path.exists(save_path):
        os.remove(save_path)
    with open(save_path, 'w', newline='') as file:
        print("The single-model multi-question fuzzing process is started!")
